  ...
  <el-drawer v-model="drawerVisible" :destroy-on-close="true" size="450px" :title="`${drawerProps.title}用户`">
    ...
    >
      <el-form-item label="用户头像" prop="avatar">
        <UploadImg
          :image-url="drawerProps.row?.avatar || ''"
          @update:image-url="drawerProps.row!.avatar = $event"
          width="135px"
          height="135px"
          :file-size="3"
        >
      ...
      <el-form-item label="用户照片" prop="photo">
        <UploadImgs
          :file-list="drawerProps.row?.photo || []"
          @update:file-list="drawerProps.row!.photo = $event"
          height="140px"
          width="140px"
          border-radius="50%"
        >
